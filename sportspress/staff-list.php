...
if ( $title ) {
	...
	$output .= '<header class="card__header">';
		...
		if ( $show_all_staff_link ) {
			$output .= '<a class="sp-staff-directory-link sp-view-all-link ' . esc_attr( implode( ' ', $card_header_btn_classes ) ) . '" href="' . get_permalink( $id ) . '">' . esc_html__( 'View all staff', 'sportspress' ) . '</a>';
		}
	$output .= '</header>';
}
