...
$i = 0;
?>
<div class="card card--has-table sp-template sp-template-event-performance sp-template-event-performance-<?php echo esc_attr( $mode ); ?><?php if ( isset( $class ) ) { echo ' ' . esc_attr( $class ); } ?>">
	<?php if ( $caption ) : ?>
									...
									if ( $key == 'position' ) :
										...
										if ( array_key_exists( $key, $row ) && $row[ $key ] !== '' ) :
											if ( 'checkbox' === $format ) :
												...
												// if mode is "Icons" don't show specific statistics and performances
												if ( $mode == 'icons' ) :
													if ( $key == 'passpercent' || $key == 'min' ) :
														continue;
													endif;
													$value = $row[ $key ];
												else :
